...
2. Ensure you have the required R packages installed:
   ```bash
   install.packages(c("GWmodel", "sf", "tmap", "tidyverse", "DataExplorer", "corrplot", "MASS"))
   ```
3. Open the R project or set your working directory to the repository folder.
4. Ensure you have the Indonesian province shapefile (```IDN.shp```) downloaded locally. Update the file path in the notebook to point to your local shapefile directory.
5. Run the EDA script in the ```scripts/``` folder first, followed by the spatial modeling notebook in the ```notebooks/``` folder.
